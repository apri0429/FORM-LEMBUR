...
import { ChevronDown, SearchMd } from '../../components/layout/icons';
...
function ManageDeptDialog({ user, allDepartments, open, onClose }) {
  ...
  useEffect(() => {
    ...
    setError('');
    axios.get(`/api/employees/user/${user.id}/departments`)
  ...
  const handleAdd = async () => {
    ...
    setError('');
    try {
      await axios.post(`/api/employees/user/${user.id}/departments`, {
  ...
  const handleRemove = async (deptId) => {
    setSaving(true);
    try {
      await axios.delete(`/api/employees/user/${user.id}/departments/${deptId}`);
      reload();
    } catch (err) {
  ...
  const handleSetPrimary = async (deptId) => {
    setSaving(true);
    try {
      await axios.patch(`/api/employees/user/${user.id}/departments/${deptId}/primary`);
      reload();
    } catch (err) {
